...
- **Villa pricing shows an entry price — `₹4.3 Cr` onwards — not a per-sq-ft rate.** Changed 9 Sep 2026 at Batman's request; the page previously showed a ₹12,999/sq.ft base (₹13,299 east) and that is **not** to be restored. The figure is the **200 Sq. Yd** entry point — **239 and 300 start higher** — so the label is *"Starting from"*, never *"all plots"* or any wording implying one price covers the collection. **₹ only — no $ conversion** (nothing should depend on an FX rate). East facing, corner and garden-view plots all cost more, and totals, specifications and payment plans remain deliberately a WhatsApp conversation — do not itemise them on the page.
